 ...
 //     if(TOTALTRADES > 0 ) { MoveStop() ; } // else { subDeleteOrder() ; }
   ...
   if(TradeAllow  )
   {
      ...
      if(total == 0 &&TOTALTRADES < MaxTrades && EntryAllow) 
      {
         ...
         {
            ticket = OrderSend(symbol,OP_SELL,Lots,SellLevel,Slippage,STOPLOSS_S,TAKEPROFIT_S,TicketComment,MAGICNUMBER,0,Red);
//ORDER_MESSAGE = TMHEAD + " RED SELL " + myHHMM+  " tp=" + DoubleToStr(TAKEPROFIT_S,Digits );   
         ...
         {
            ticket = OrderSend(symbol,OP_BUY,Lots,BuyLevel,Slippage,STOPLOSS_L,TAKEPROFIT_L,TicketComment,MAGICNUMBER,0,Green);          
